...
if [ $? -ne 0 ]; then
    ...
    echo -e "mysql already installed ... $Y skipping $N" | tee -a $LOGS_FILE
fi
...
if [ $? -ne 0 ]; then
   ...
   VALIDATE $? "nginx"
    ...
    echo -e "nginx already installed ... $Y skipping $N" | tee -a $LOGS_FILE   
fi
...
if [ $? -ne 0 ]; then
   ...
   VALIDATE $? "python3" 
    ...
    echo -e "python3 already installed ... $Y skipping $N" | tee -a $LOGS_FILE  
fi   